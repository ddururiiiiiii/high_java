...
public class T07_WildcardTest {
	 ...
	 */
	public static void listWrokerCourseInfo(Course<? super Worker> course) {
		System.out.println(course.getName()
				+ "수강생 : "
				+ Arrays.toString(course.getStudents()));
	}
	...
	public static void main(String[] args) {
		...
		personCourse.add(new HighStudent("학생1"));
		...
		studentCourse.add(new HighStudent("고등학생학생1"));
		...
		listWrokerCourseInfo(personCourse);
		listWrokerCourseInfo(workerCourse);
		//listWrokerCourseInfo(studentCourse);
		//listWrokerCourseInfo(highStudentCourse);
		System.out.println("----------------------------------------------");
	}
}
...
class Course<T> {
	...
	public Course(String name, int capacity) {
		...
		//students = new T[capacity]; //제너릭 배열 생성 실패
									// => new 연산자는 생성할 객체의 타입이 명확히 정의되어야 객채생성 가능.
	...
	public void add(T t) {
		for (int i = 0; i < students.length; i++) {
			if (students[i] == null) { // 아직 등록되지 않은 빈자리인지 확인
				students[i] = t;
				break;
			}
		}
